...
#[tokio::test]
async fn reflectors_start_outside_a_dioxus_runtime() {
    // The web host starts the reflectors from a plain tokio process, where a
    // global signal has no Dioxus runtime to resolve against. Starting them must
    // therefore not panic, and the reflectors must be live.
    let started = openkite::state::live::start(unreachable_client());
